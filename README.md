...
+ [`2.11.0`,`latest` (2.11.0/Dockerfile)](https://github.com/danlynn/ember-cli/blob/2.11.0/Dockerfile)
...
ember-cli 2.11.0 + node 6.9.4 + npm 3.10.10 + bower 1.8.0 + phantomjs 2.1.7 + watchman 3.5.0
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp -p 4200:4200 danlynn/ember-cli:2.11.0
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:2.11.0 ember test
```
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:2.11.0 bash
```
...
1. Create new project dir and add a docker-compose.yml file similar to the following:
   ...
   ```
   ember: &defaults
     image: danlynn/ember-cli:2.11.0
     volumes:
       - .:/myapp
...
3. Start the ember server:
   ...
   ```
   $ docker run --rm --privileged --entrypoint sysctl danlynn/ember-cli:2.11.0 -w fs.inotify.max_user_watches=524288
   ```
...
Basically put `docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.0` before any command you run.

Example:

```
$ docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.0 npm install
$ docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.0 bower --allow-root install bootstrap
$ docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.0 ember generate model user
$ docker run -ti -v $(pwd):/myapp -p 4200:4200 danlynn/ember-cli:2.11.0
```

Alternatively, you could simply launch into a bash shell and execute the commands in the normal fashion:

```
$ docker run -ti -v $(pwd):/myapp -p 4200:4200 danlynn/ember-cli:2.11.0 bash
