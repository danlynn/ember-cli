...
+ [`3.6.1`,`3.6.1-node_10.15`,`latest` (3.6.1/Dockerfile)](https://github.com/danlynn/ember-cli/blob/3.6.1/Dockerfile)
...
`ember-cli 3.6.1 + node 10.15.0/11.6.0 + npm 6.4.1/6.5.0-next.0 + bower 1.8.4 + yarn 1.12.3 + chrome 71.0.3578.98 + watchman 4.9.0`
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp -p 4200:4200 -p 7020:7020 danlynn/ember-cli:3.6.1
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:3.6.1 ember test
```
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:3.6.1 bash
```
...
3. Start the ember server:
   ...
   ```
   $ docker run --rm --privileged danlynn/ember-cli:3.6.1 sysctl -w fs.inotify.max_user_watches=524288
   ```
...
Basically put `docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:3.6.1` before any command you run.

Example:

```
$ docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:3.6.1 npm install
$ docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:3.6.1 bower --allow-root install bootstrap
$ docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:3.6.1 ember generate model user
$ docker run --rm -ti -v $(pwd):/myapp -p 4200:4200 -p 7020:7020 -p 7357:7357 danlynn/ember-cli:3.6.1
```
...
$ mkdir new_ember_app
$ cd new_ember_app
$ docker run --rm -it -v $(pwd):/myapp -p 4200:4200 -p 7020:7020 -p 7357:7357 danlynn/ember-cli:3.6.1 bash
