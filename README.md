...
+ [`2.17.1`,`2.17.1-node_8.9`,`latest` (2.17.1/Dockerfile)](https://github.com/danlynn/ember-cli/blob/2.17.1/Dockerfile)
...
`ember-cli 2.17.1 + node 8.9.3/9.3.0 + npm 5.5.1/5.5.1 + bower 1.8.0 + yarn 1.3.2 + phantomjs 2.1.1 + chrome 62.0.3202.94 + watchman 4.7.0`
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp -p 4200:4200 -p 7020:7020 danlynn/ember-cli:2.17.1
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:2.17.1 ember test
```
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:2.17.1 bash
```
...
1. Create new project dir and add a docker-compose.yml file similar to the following:
   ...
   ```
   ember: &defaults
     image: danlynn/ember-cli:2.17.1
     volumes:
       - .:/myapp
...
3. Start the ember server:
   ...
   ```
   $ docker run --rm --privileged danlynn/ember-cli:2.17.1 sysctl -w fs.inotify.max_user_watches=524288
   ```
...
Basically put `docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:2.17.1` before any command you run.

Example:

```
$ docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:2.17.1 npm install
$ docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:2.17.1 bower --allow-root install bootstrap
$ docker run --rm -ti -v $(pwd):/myapp danlynn/ember-cli:2.17.1 ember generate model user
$ docker run --rm -ti -v $(pwd):/myapp -p 4200:4200 -p 7020:7020 -p 7357:7357 danlynn/ember-cli:2.17.1
```
...
$ mkdir new_ember_app
$ cd new_ember_app
$ docker run --rm -it -v $(pwd):/myapp -p 4200:4200 -p 7020:7020 -p 7357:7357 danlynn/ember-cli:2.17.1 bash
