...
+ [`1.13.1`,`latest` (1.13.1/Dockerfile)](https://github.com/danlynn/ember-cli/blob/1.13.1/Dockerfile)
...
ember-cli v1.13.1 + node 0.12.7 + npm 2.12.1 + bower 1.7.1 + phantomjs 1.9.19 + watchman 3.5.0
...
1. Create new project dir and add a docker-compose.yml file similar to the following:
   ...
   ```
   ember: &defaults
     image: danlynn/ember-cli:1.13.1
     volumes:
       - .:/myapp
...
3. Start the ember server:
   ...
   ```
   $ docker run --rm --privileged --entrypoint sysctl danlynn/ember-cli:1.13.1 -w fs.inotify.max_user_watches=524288
   ```
