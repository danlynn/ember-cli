...
ember-cli v2.9.1 + node 4.2.3 + npm 2.14.7 + bower 1.7.9 + phantomjs 2.1.7 + watchman 3.5.0
...
3. Start the ember server:
   ...
   ```
   $ docker run --rm --privileged --entrypoint sysctl danlynn/ember-cli:2.9.1 -w fs.inotify.max_user_watches=524288
   ```
