...
+ [`2.11.1`,`latest` (2.11.1/Dockerfile)](https://github.com/danlynn/ember-cli/blob/2.11.1/Dockerfile)
+ [`2.11.1-node_6.9` (2.11.1-node_6.9/Dockerfile)](https://github.com/danlynn/ember-cli/blob/2.11.1-node_6.9/Dockerfile)
...
ember-cli 2.11.1 + node 4.6.2/6.9.4 + npm 2.15.11/3.10.10 + bower 1.8.0 + phantomjs 2.1.7 + watchman 3.5.0
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp -p 4200:4200 -p 49153:49153 danlynn/ember-cli:2.11.1
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:2.11.1 ember test
```
...
NEW: $ docker run -ti --rm -v $(pwd):/myapp danlynn/ember-cli:2.11.1 bash
```
...
3. Start the ember server:
   ...
   ```
   $ docker run --rm --privileged danlynn/ember-cli:2.11.1 sysctl -w fs.inotify.max_user_watches=524288
   ```
...
Basically put `docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.1` before any command you run.

Example:

```
$ docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.1 npm install
$ docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.1 bower --allow-root install bootstrap
$ docker run -ti -v $(pwd):/myapp danlynn/ember-cli:2.11.1 ember generate model user
$ docker run -ti -v $(pwd):/myapp -p 4200:4200 -p 49153:49153 danlynn/ember-cli:2.11.1
```

Alternatively, you could simply launch into a bash shell and execute the commands in the normal fashion:

```
$ docker run -ti -v $(pwd):/myapp -p 4200:4200 -p 49153:49153 danlynn/ember-cli:2.11.1 bash
